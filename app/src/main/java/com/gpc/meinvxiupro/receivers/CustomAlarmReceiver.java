...
public class CustomAlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals(Constant.BroadcastReceiverAction.BOOT_COMPLETE)) {
            //set alarm
            WallpaperUtils.setAlarmAutoSetWallpaper(context);
        } else if (intent.getAction().equals(Constant.BroadcastReceiverAction.AUTO_SET_WALLPAPER)) {
